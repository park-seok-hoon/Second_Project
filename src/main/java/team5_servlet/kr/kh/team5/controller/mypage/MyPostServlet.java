...
public class MyPostServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalCount = boardService.getTotalCount(cri,id);
		...
		ArrayList<BoardVO> list = boardService.getBoardList(cri,id);
		System.out.println(list);
		//화면에 게시글 리스트를 전송 : 화면에서 사용할 이름 - boardList
		request.setAttribute("boardList", list);
		request.getRequestDispatcher("/WEB-INF/mypage/myPost.jsp").forward(request, response);
	}
